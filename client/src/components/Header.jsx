...
export default function Header({setPage, logOut, userData}) {
	const loggedOut = (
		<>
			<NavButton signup onClick={() => setPage('signup')}>
				Create Account
			</NavButton>
			<NavButton login onClick={() => setPage('login')}>
				Log In
			</NavButton>
		</>
	);
	const loggedIn = (
		<>
			<NavButton signup onClick={() => setPage('dashboard')}>
				Your Dashboard
			</NavButton>
			<NavButton login onClick={() => logOut()}>
				Log Out
			</NavButton>
		</>
	);

	const buttonRender = userData ? loggedIn : loggedOut;
